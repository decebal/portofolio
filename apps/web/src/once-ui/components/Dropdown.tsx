  ...
  (
    ...
    useEffect(() => {
      if (internalRef.current && focusedIndex !== null) {
        const focusedOption = internalRef.current.querySelectorAll<HTMLElement>(
          `.${styles.option}`,
        )[focusedIndex];
        if (focusedOption) {
    ...
    useEffect(() => {
      if (typedChars.length > 0) {
        const matchIndex = options.findIndex((option) =>
          option.label.toLowerCase().startsWith(typedChars.toLowerCase()),
        );
        if (matchIndex !== -1) {
      ...
      >
        ...
        {options.map((option, index) => (
          ...
          <React.Fragment key={index}>
            ...
            >
              {option.hasPrefix && (
                <Flex className={styles.prefix}>{option.hasPrefix}</Flex>
              )}
              <Flex
              ...
              {option.hasSuffix && (
                <Flex className={styles.suffix}>{option.hasSuffix}</Flex>
              )}
            </Flex>
